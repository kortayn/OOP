...
class TestClass
{
    ...
    //При присваивании используется подход copy and swap
    TestClass& operator=(TestClass& object)
    {
        if (&object != this) 
        {
            //Создаётся копия объекта который будет присваиваться
            TestClass temp_object(object);
            //Если при копировании произойдёт ошибка - текущий объект сохранит своё состояние
